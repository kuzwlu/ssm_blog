	...
	<table width="99%" border="0" cellspacing="0" cellpadding="0" id="searchmain">
	  ...
	  <tr>
	    ...
	    <table width="100%" border="0" cellspacing="0" cellpadding="0" id="search">
	  		<tr>
	  		  ...
	  		  <a href="article/article_info" target="mainFrame" onFocus="this.blur()" class="add">新增文章</a></td>
	  		</tr>
		</table>
	    </td>
	  </tr>
	  <tr>
	    ...
	    <table width="100%" border="0" cellspacing="0" cellpadding="0" id="main-tab">
	      ...
	      <c:forEach items="${requestScope.articleList}" var="p">
	        ...
	        <c:forEach items="${requestScope.catalogList }" var="c">
	        	<c:if test="${p.caid == c.caid }">
	        		<a href="catalog/search?caname=${c.caname }" target="mainFrame" onFocus="this.blur()" class="add"/>
	        		${c.caname}</a></td>
	        	</c:if>
	        </c:forEach>
	        <td align="center" valign="middle" class="borderright borderbottom"><a href="article/article_update/${p.arid }" target="mainFrame" onFocus="this.blur()" class="add">${p.artitle }</a></td>
	        ...
	        <td align="center" valign="middle" class="borderbottom">
	        	<a href="article/article_update/${p.arid }" target="mainFrame" onFocus="this.blur()" class="add">编辑</a>
	        	
	        	<span class="gray">&nbsp;|&nbsp;</span>
	        	<a href="article/exchange/${p.arnumber }"  target="mainFrame" onFocus="this.blur()" class="add" />上移</a>
	        	<span class="gray">&nbsp;|&nbsp;</span>
	  ...
	  <tr>
	    <td align="left" valign="top" class="fenye">共${requestScope.pageSize }条数据&nbsp;&nbsp;${requestScope.startPage}/${requestScope.lastPage}页&nbsp;&nbsp;
	    <a href="article/list/1" target="mainFrame" onFocus="this.blur()">首页</a>&nbsp;&nbsp;
	    <c:choose>
	    	<c:when test="${requestScope.startPage ==1 }">
	    		<a href="article/list/1" target="mainFrame" onFocus="this.blur()">上一页</a>&nbsp;&nbsp;
	    	</c:when>
	    	<c:otherwise>
	    		<a href="article/list/${requestScope.startPage - 1 }" target="mainFrame" onFocus="this.blur()">上一页</a>&nbsp;&nbsp;
	    	</c:otherwise>
	    </c:choose>
	    <c:choose>
	    	<c:when test="${requestScope.startPage >= lastPage }">
	    		<a href="article/list/${requestScope.startPage}" target="mainFrame" onFocus="this.blur()">下一页</a>&nbsp;&nbsp;
	    	</c:when>
	    	<c:otherwise>
	    		<a href="article/list/${requestScope.startPage + 1 }" target="mainFrame" onFocus="this.blur()">下一页</a>&nbsp;&nbsp;
	    	</c:otherwise>
	    </c:choose>
	    <a href="article/list/${requestScope.lastPage}" target="mainFrame" onFocus="this.blur()">尾页</a></td>
	  </tr>
	  ...
	  function del(arid){
			swal({
				...
				if (isConfirm === true) {
					window.location.href='article/delete/'+arid;
					swal(
					  '删除成功',
					  '',
					  'success'
